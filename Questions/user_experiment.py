...
def generate_survey(dataset_name, direction, df, X, subjects, smooth_lambda=0.05, n=75, language='Eng'):
    ...
    # baseline_estimator = VerbSelectBaselineMRREstimator(classes_, smooth_lambda)
    # baseline_estimator.fit(X, y=encoding)  # MLP
...
def main():
    rising_verbs_WSJ, falling_verbs_WSJ = preprocess('../Data/bllip_triples', VERBS)
    rising_verbs_Returs, falling_verbs_Reuter = preprocess('../Data/Reuters_triples', VERBS_Reuters)
    rising_verbs_Chinese, falling_verbs_Chinese = preprocess('../Data/Chinese_news_triples', VERBS_Chinese)
    # WSJ
    print("WSJ: ")
