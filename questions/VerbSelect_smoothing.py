...
class VerbSelectBetaMRREstimator(VerbSelectBase):
    ...
    def p_verb(self, x):
        """
        given a percent, get the verb with the highest probability
        :param x: percent
        :return: verb
        """
        y_score = pd.Series([self.p_posterior(w, x) for w in self.verbs], index=self.verbs)
        p_verb = y_score.idxmax()
        return p_verb
